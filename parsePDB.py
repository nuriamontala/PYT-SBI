...
def pdb2fasta(pdb_id, pdb_directory, fasta_directory, min_length=30, max_chains=6):
    """Extract protein sequence and save the FASTA in given directory. Remove chains with <30 residues and pdb with >6 chains."""
...
def get_offset_from_pdb(pdb_id, pdb_directory):
    """Extract residue ids offsets per chain. Return a dictionary {chain_id: offset}."""
...
def get_binding_sites(pdb_id, pdb_site_directory):
    """Assign 1 to binding sites positions (PDBbind), 0 otherwise, return a dictionary with keys (res_id,chain_id,res_name)."""
